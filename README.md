...
The project includes a comprehensive suite of unit and integration tests that validate each component of the bias detection and mitigation system. These tests ensure the robustness, accuracy, and reliability of the system across different scenarios.

### Test Location and Organization

The tests are located at the end of the main `clasificacionbinaria.ipynb` file, organized into six main suites:

1. **DataLoader Tests** (`test_data_loader`): Validates data loading and preparation.
2. **BiasMetricsCalculator Tests** (`test_bias_metrics_calculator`): Verifies fairness metric calculations.
3. **RealTimeBiasMonitor Tests** (`test_realtime_monitor`): Evaluates real-time monitoring.
4. **Mitigation Techniques Tests** (`test_mitigation_techniques`): Validates the execution of mitigation techniques.
5. **Intersectional Tests** (`test_intersectional_cases`): Evaluates complex intersectional bias cases.
6. **Integrated System Tests** (`test_integrated_system`): Validates the complete system flow.

### Running the Tests

**IMPORTANT:** Due to performance constraints of the environment (Google Colab/Jupyter Notebook), the tests are **commented out by default** in the main file. To run them, you must:

1. **Run the main system first**: Ensure all upper cells of the notebook have been executed successfully, including dependency installation and library imports.
2. **Uncomment individual tests**: Locate the specific test function you wish to run (from approximately line 3200 onwards) and uncomment it by removing the `#` symbol at the beginning of each line.

### Execution Methods

#### Option 1: Individual Test Execution
To run specific tests without overloading system resources:
```python
# Run DataLoader tests
test_data_loader()

# Run fairness metrics tests
test_bias_metrics_calculator()

# Run real-time monitor tests
test_realtime_monitor()

# Run mitigation techniques tests
test_mitigation_techniques()

# Run intersectional cases tests
test_intersectional_cases()
